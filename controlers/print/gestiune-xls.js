...
const period = `${formatedDateToShow(data[0].date).split('ora')[0]} - ${formatedDateToShow(data[data.length - 1].date).split('ora')[0]}`
  ...
  const workbook = new ExcelJS.Workbook();
        ...
        data.forEach((el, i) => {
          const head =  worksheet.addRow([`${i+1}`,`${el.date.toISOString().split('T')[0]}`,`Sold initial`,`${el.in11}`,`${0}`,`${el.in21}`, `${0}` ])
            head.eachCell((cell) => {
                cell.font = {
                size: 13,
                bold: true,
            ...
            el.entries.forEach((el, i) => {
               ...
               const e = worksheet.addRow([`${i+1}`,`${el.date.toISOString().split('T')[0]}`,`${el.description}`,`${in11}`,`${out11}`, `${in21}`, `${out21}`])
            //    const sum = e.getCell(5)
            //    sum.font =  { color:  {argb: el.tip === 'income' ?  'FF00B050' : 'FF0000'} }
            });
         
           const foot =  worksheet.addRow(['',`${el.date.toISOString().split('T')[0]}`,`Sold final`,`${0}`,`${el.out11}`, `${0}`, `${el.out21}`])
