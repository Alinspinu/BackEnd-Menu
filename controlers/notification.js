...
module.exports.addNotification = async(req, res) => {
    ...
    try{
        ...
        let userIds = []
        if(userId === 'all'){
            userIds = (await User.find({'employee.active': true}).select('_id')).map(u => u._id)
        } else {
            userIds = userId
        }
      ...
      if (reservations.length > 0) {
        const notificationPromises = reservations.map(async (reservation) => {
            const notif = {
              ...
              message: `Avem o rezevare ${formatedDateToShow(reservation.date)}, ${reservation.client.name}, ${reservation.guests} persoane, ${reservation.position}!`
            };
            try {
