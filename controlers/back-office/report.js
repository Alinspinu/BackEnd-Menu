...
module.exports.getAllReports = async(req, res, next) => {
    try{
        ...
        const reports = await Report.find({locatie: loc, salePoint: point, status: 'new', period: { $exists: false }}).sort({day: -1}).lean()
                // .select('departaments diverse day impairment ingsValue period profit')
