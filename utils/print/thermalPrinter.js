...
async function createBillForPrinter(order, logoUrl = ' ', qrUrl = ' ') {
  ...
  if(order.locatie === '690c818c21500095430c613f' ){
    if(order.tips === 0){
      ...
      parts.push(lf)
      parts.push(Buffer.from(`10% (${round(0.1 * order.total)})=${order.total + round(0.1 * order.total)} Lei []\n`, 'ascii'))
      parts.push(lf)
      parts.push(Buffer.from(`15% (${round(0.15 * order.total)})=${order.total + round(0.15 * order.total)} Lei []\n`, 'ascii'))
      parts.push(lf)
